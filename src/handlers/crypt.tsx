...
interface hashI {
    ...
    hash(): string
    blowfish(): string
    EKSBlowfish(): [string[], string[][]]
    keyExpand(P: string[], S: string[][], additive: string): [string[], string[][]]
}
...
class hasher implements hashI {
    ...
    private P = [ "243f6a88", "85a308d3", "13198a2e", "03707344", "a4093822",
    ...
    bCrypt() : string {
        const [P, S] = this.EKSBlowfish() // P is subkey array, S is substitution boxes

        let rpText = "PausingForExpenditures"

        for(let i = 0; i < 32; i++) {
            rpText = this.blowfish(P, S, rpText)
        }

        return this.cost + this.salt + rpText
    }

    private blowfish(P : string[], S: string[][], text: string) : string {

    }

    private EKSBlowfish() : [string[], string[][]] {
        let [P, S]: [string[], string[][]] = [this.P, this.S]

        const [a, b] = this.keyExpand(P, S, this.pass)
        P = a
        S = b
        const [c, d] = this.keyExpand(P, S, this.salt)
        P = c
        S = d
    ...
    private keyExpand(P: string[], S: string[][], additive: string) : [string[], string[][]] {
        // simple hex xor gate
        const xor = (a: string, b: string) => {
            const strA : number = parseInt(a, 16)
            const strB : number = parseInt(b, 16)

            const ans = strA ^ strB
            return ans.toString(16)
        }

        // mixes password into the P subkeys array
        for (let i = 1; i <= 18; i++) {
            P[i] = xor(P[i], this.pass + this.pass.substring(0, 18 - this.pass.length))


        }

        // splits 128-bit salt into 
    }
}
